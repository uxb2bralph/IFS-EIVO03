...
{
    public class C0401ResultWatcher : InvoiceWatcher
    {
        public C0401ResultWatcher(String fullPath)
            : base(fullPath)
        {
            _ResponsedPath = fullPath + "(ResponseCascade)";
                        ...
                        foreach (var item in dataNo)
                        {
                            if (item?.Length == 10)
                            {
                                ...
                                if (invoice != null)
                                {
                                    ...
                                    Console.WriteLine($"Invoice Good:{item}");
                                    continue;
                                }
                            }
