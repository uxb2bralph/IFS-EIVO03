...
{
    public class InvoiceTransferManagerV2ForProxy : ITransferManager
    {
        private InvoiceWatcher _InvoiceWatcher;
        private InvoiceWatcher _PreInvoiceWatcher;
        private CsvInvoiceWatcher _CsvInvoiceWatcher;
        ...
        public void EnableAll(String fullPath)
        {
            ...
            _InvoiceWatcher = new InvoiceWatcherV2ForProxy(Path.Combine(fullPath, Settings.Default.UploadSellerInvoiceFolder));
            _InvoiceWatcher.StartUp();

            _CsvInvoiceWatcher = new CsvInvoiceWatcherV2ForProxy(Path.Combine(fullPath, Settings.Default.UploadCsvInvoiceFolder));
            _CsvInvoiceWatcher.StartUp();

            _PreInvoiceWatcher = new PreInvoiceWatcherV2ForProxy(Path.Combine(fullPath, Settings.Default.UploadPreInvoiceFolder));
            _PreInvoiceWatcher.StartUp();

            _CancellationWatcher = new InvoiceCancellationWatcherV2ForProxy(Path.Combine(fullPath, Settings.Default.UploadInvoiceCancellationFolder));
            _CancellationWatcher.StartUp();

            _AllowanceWatcher = new AllowanceWatcherV2ForProxy(Path.Combine(fullPath, Settings.Default.UploadAllowanceFolder));
            _AllowanceWatcher.StartUp();

            _AllowanceCancellationWatcher = new AllowanceCancellationWatcherV2ForProxy(Path.Combine(fullPath, Settings.Default.UploadAllowanceCancellationFolder));
