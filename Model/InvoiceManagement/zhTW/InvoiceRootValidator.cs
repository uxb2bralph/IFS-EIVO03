...
namespace Model.InvoiceManagement.zhTW
{
    public static partial class InvoiceRootValidator
    {
        ...
        public static Exception CheckBusiness(this InvoiceRootInvoice invItem, GenericManager<EIVOEntityDataContext> mgr, OrganizationToken owner, out Organization seller)
        {
            ...
            if (seller == null)
            {
                return new Exception(String.Format("賣方為非註冊營業人,開立人統一編號:{0}，TAG:< SellerId />", invItem.SellerId));
            }
            ...
            {
                return new Exception(String.Format("開立人已註記停用,開立人統一編號:{0}，TAG:<SellerId />", invItem.SellerId));
            }
        ...
        public static Exception CheckAppliedInvoiceNo(this InvoiceRootInvoice invItem, GenericManager<EIVOEntityDataContext> mgr, Organization seller,DateTime invoiceDate,String trackCode,String invoiceNo)
        {
            using (TrackNoManager trackMgr = new TrackNoManager(mgr, seller.CompanyID))
            {
                var item = trackMgr.GetAppliedInterval(invoiceDate, trackCode, int.Parse(invoiceNo));

                if (item == null)
                {
                    return new Exception(String.Format("發票號碼錯誤:{0}，TAG:< InvoicNumber />", invItem.InvoiceNumber));
                }
            }
            return null;
        }
        ...
        public static Exception CheckBusiness_Proxy(this InvoiceRootInvoice invItem, GenericManager<EIVOEntityDataContext> mgr, OrganizationToken owner, out Organization seller)
        {
            ...
            {
                return new Exception(String.Format("上傳營業人並無登記在此發票開立代理營業人底下:{0}，TAG:< SellerId />；代理發票開立營業人:{1}", invItem.SellerId, proxyOrg.ReceiptNo));
            }
        ...
        public static Exception CheckAmount(this InvoiceRootInvoice invItem)
        {
            ...
            {
                if (!invItem.CustomsClearanceMark.HasValue)
                {
            ...
            else if (!invItem.CustomsClearanceMark.HasValue)
            {
