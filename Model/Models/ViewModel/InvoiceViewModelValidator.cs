        ...
        where TEntity : class, new()
    {
        ...
        protected virtual Exception checkInvoice()
        {
            _newItem = new InvoiceItem
            {
                ...
                PrintMark = _carrier == null && _donation == null ? "Y" : "N",
            };
            ...
            if (_invItem.TrackCode == null || _invItem.No == null)
            {
                try
                {
                    using (TrackNoManager trackNoMgr = new TrackNoManager(_mgr, _seller.CompanyID))
                    {
                        ...
                        {
                            trackNoMgr.ApplyInvoiceTypeIndication(Naming.InvoiceTypeDefinition.特種稅額計算之電子發票);
                        }
        ...
        protected virtual Exception checkBusinessDetails()
        {
            _buyer = new InvoiceBuyer
            {
                ...
                Name = String.IsNullOrEmpty(_invItem.BuyerReceiptNo) ? ((int)4).CreateRandomStringCode() : _invItem.BuyerName,
            ...
            if (_invItem.B2BRelation == true && _seller.MasterRelation.Count > 0)
            {
