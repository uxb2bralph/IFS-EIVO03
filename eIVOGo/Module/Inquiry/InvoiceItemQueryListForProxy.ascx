<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="InvoiceItemQueryListForProxy.ascx.cs"
    Inherits="eIVOGo.Module.Inquiry.InvoiceItemQueryListForProxy" %>
...
<asp:GridView ID="gvEntity" runat="server" AutoGenerateColumns="False" Width="100%"
    ...
    <Columns>
        ...
        <asp:TemplateField HeaderText="開立發票營業人">
            <ItemTemplate>
                <%# ((InvoiceItem)Container.DataItem).InvoiceSeller.Organization.CompanyName%></ItemTemplate>
            <ItemStyle HorizontalAlign="Center" />
        </asp:TemplateField>
        <asp:TemplateField HeaderText="統編">
            <ItemTemplate>
                <%# ((InvoiceItem)Container.DataItem).InvoiceSeller.Organization.ReceiptNo%></ItemTemplate>
            <ItemStyle HorizontalAlign="Center" />
        </asp:TemplateField>
        ...
        <asp:TemplateField HeaderText="買受人統編"  FooterText="總計金額："> 
            <ItemTemplate><%# ((InvoiceItem)Container.DataItem).InvoiceBuyer.ReceiptNo.Equals("0000000000") ? "N/A" : ((InvoiceItem)Container.DataItem).InvoiceBuyer.ReceiptNo %></ItemTemplate>  
            <ItemStyle HorizontalAlign="Center"  />
        ...
        <asp:TemplateField HeaderText="簡訊通知" > <ItemTemplate><%# ((InvoiceItem)Container.DataItem).CDS_Document.SMSNotificationLogs.Any() ? "是" : "否"%></ItemTemplate>  
            <ItemStyle HorizontalAlign="Center"  />
        </asp:TemplateField>
        <asp:TemplateField HeaderText="載具資訊" > <ItemTemplate><%# ((InvoiceItem)Container.DataItem).InvoiceCarrier == null ? "" : ((InvoiceItem)Container.DataItem).InvoiceCarrier.CarrierNo%></ItemTemplate>  
            <ItemStyle HorizontalAlign="Center"  />
        </asp:TemplateField>
    </Columns>
    <FooterStyle />
