...
<asp:PlaceHolder ID="phFields" runat="server">
    ...
    <script>
        ...
        fields[fields.length] =
        {
            ...
            "width": "120",
            "align": "right",
            footerTemplate: function () { return "<%= String.Format("{0:##,###,###,##0.##}", Select().Sum(i => i.InvoiceAllowance.TotalAmount)) %>"; }
        };
    </script>
...
<script runat="server">
    ...
    public override IEnumerable<object> GetCsvResult(IEnumerable<CDS_Document> items)
    {
        return items.Select(d => new
        {
            ...
            未稅金額 = String.Format("{0:##,###,###,##0.##}", d.InvoiceAllowance.TotalAmount - d.InvoiceAllowance.TaxAmount),
            稅額 = String.Format("{0:##,###,###,##0.##}", d.InvoiceAllowance.TaxAmount),
            含稅金額 = String.Format("{0:##,###,###,##0.##}", d.InvoiceAllowance.TotalAmount),
