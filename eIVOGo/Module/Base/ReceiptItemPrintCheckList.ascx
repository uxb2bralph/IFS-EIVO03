            ...
            <asp:TemplateField HeaderText="列印狀態">
                <ItemTemplate>
                    <%--<asp:Label ID="Printstatus" runat="server" Text="<%# checkPrintStatus(((CDS_Document)Container.DataItem).ReceiptItem.CDS_Document) %>"
                        EnableViewState="false"></asp:Label>--%>
                    <%# ((CDS_Document)Container.DataItem).DocumentPrintLog.Any(l => l.TypeID == (int)Model.Locale.Naming.DocumentTypeDefinition.E_Receipt) ? "已列印" : ((CDS_Document)Container.DataItem).DocumentPrintQueue != null ? "已列印" : "未列印"%>
                </ItemTemplate>
                <ItemStyle HorizontalAlign="Center" />
            </asp:TemplateField>
        </Columns>
...
<script runat="server">
    ...
    {
        return document.DocumentPrintLog.Any(l => l.TypeID == (int)Model.Locale.Naming.DocumentTypeDefinition.E_Receipt) ? "已列印" : "未列印";
    }
