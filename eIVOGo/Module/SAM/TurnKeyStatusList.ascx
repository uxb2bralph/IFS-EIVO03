...
<br />
    ...
    <Columns>
        ...
        <asp:TemplateField HeaderText="金額">
            <ItemTemplate>
                <%#_Invoice !=null?String.Format("{0:##,###,###,##0.##}", _Invoice.InvoiceAmountType.TotalAmount):
                String.Format("{0:##,###,###,##0.##}", _InvoiceAllowance.TotalAmount)%></ItemTemplate>
            <ItemStyle HorizontalAlign="Center" />
            <FooterTemplate>
...
<cc1:V_LogsDataSource ID="dsEntity" runat="server">
</cc1:V_LogsDataSource>
<cc2:InvoiceDataSource ID="dsEntityoice" runat="server">
...
String getinvoiceitem(String invoiceno)
{
    ...
    {
        _Invoice = dsEntityoice.CreateDataManager().GetTable<InvoiceItem>()
        ...
        if (SearchItem.SelectedValue.Equals("C0401"))
            ...
            return dsEntityoice.CreateDataManager().GetTable<DerivedDocument>().Where(d => d.SourceID == _Invoice.InvoiceID).FirstOrDefault().DocID.ToString();
        
    }
    else
    {
        _InvoiceAllowance = dsEntityoice.CreateDataManager().GetTable<InvoiceAllowance>()
        ...
        return _InvoiceAllowance.AllowanceID.ToString();
            ...
            return dsEntityoice.CreateDataManager().GetTable<DerivedDocument>().Where(d => d.SourceID == _InvoiceAllowance.AllowanceID).FirstOrDefault().DocID.ToString();
