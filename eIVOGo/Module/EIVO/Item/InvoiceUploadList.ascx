...
<asp:GridView ID="gvEntity" runat="server" AutoGenerateColumns="False" Width="100%"
    ...
    <Columns>
        ...
        <asp:TemplateField HeaderText="未稅金額" FooterText="總計">
            <ItemTemplate>
                <%# ((GoogleInvoiceItem)Container.DataItem).Invoice!=null? ((GoogleInvoiceItem)Container.DataItem).Invoice.InvoiceAmountType!=null ? String.Format("{0:##,###,###,##0.##}", ((GoogleInvoiceItem)Container.DataItem).Invoice.InvoiceAmountType.SalesAmount) : "" :"" %></ItemTemplate>
            <ItemStyle HorizontalAlign="Right" />
        </asp:TemplateField>
        <asp:TemplateField HeaderText="稅額">
            <ItemTemplate>
                <%# ((GoogleInvoiceItem)Container.DataItem).Invoice!=null?((GoogleInvoiceItem)Container.DataItem).Invoice.InvoiceAmountType!=null ? String.Format("{0:##,###,###,##0.##}", ((GoogleInvoiceItem)Container.DataItem).Invoice.InvoiceAmountType.TaxAmount) : "":"" %></ItemTemplate>
            <ItemStyle HorizontalAlign="Right" />
            <FooterTemplate>
        ...
        <asp:TemplateField HeaderText="含稅金額">
            <ItemTemplate>
                <%# ((GoogleInvoiceItem)Container.DataItem).Invoice!=null?((GoogleInvoiceItem)Container.DataItem).Invoice.InvoiceAmountType!=null ? String.Format("{0:##,###,###,##0.##}", ((GoogleInvoiceItem)Container.DataItem).Invoice.InvoiceAmountType.TotalAmount) : "":"" %></ItemTemplate>
            <ItemStyle HorizontalAlign="Right" />
            <FooterTemplate>
