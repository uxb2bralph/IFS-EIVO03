        ...
        if (File.Exists(keyCodeFile))
        {
            if (!String.IsNullOrEmpty(File.ReadAllText(keyCodeFile)))
            {
                ...
                if (ar != null && ar.Count() > 0)
                {
                    ...
                    LiteralControl lc = new LiteralControl(String.Format("<iframe src='{0}?printBack={1}' height='0' width='0'></iframe>", VirtualPathUtility.ToAbsolute("~/DataView/PrintC0401AsPDF"), Request["printBack"]));
