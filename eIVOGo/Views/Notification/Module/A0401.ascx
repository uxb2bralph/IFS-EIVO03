...
已開出您本期發票，請點擊下述電子發票接收連結或登入平台執行接收發票作業，<br />
<a href="<%= $"{Uxnet.Web.Properties.Settings.Default.HostUrl}{VirtualPathUtility.ToAbsolute("~/InvoiceBusiness/CommitToReceive")}?keyID={HttpUtility.UrlEncode(_model.InvoiceID.EncryptKey())}" %>">確認接收電子發票(<%= _model.TrackCode %><%= _model.No %>)</a><br />
請於附件(PDF)列印本期的電子發票，如附件列印有問題再麻煩請您登入<br />
『電子發票服務平台』進行後續列印作業，登入電子發票網址 <a href="<%= $"{Uxnet.Web.Properties.Settings.Default.HostUrl}" %>"><%= $"{Uxnet.Web.Properties.Settings.Default.HostUrl}" %></a>。<br />
