        ...
        protected override void processFile(String invFile)
        {
            ...
            finally
            {
                if (result.Automation != null)
                {
                    ...
                    String gpgName = Path.Combine(_ResponsedPath, responseName + ".xml.gpg");
                    responseName = Path.Combine(Logger.LogDailyPath, responseName + ".xml");
                    auto.ConvertToXml().Save(responseName);

                    String args = $"{responseName} {gpgName}";
                    Process proc = Process.Start(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "pgp_encrypt.bat"), args);
                }
            }
        }

        private void processPGP(String invoiceFile)
        {
